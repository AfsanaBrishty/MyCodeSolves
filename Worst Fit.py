# Memory Management Algorithm(Worst Fit) in Python by Afsana Afrin Brishty#
def worstFit(Memory_holes, m, Memory_req, n):
    allocation = [-1] * n
    external_frag = 0
    for i in range(n):
        wstIdx = -1
        for j in range(m):
            if Memory_holes[j] >= Memory_req[i]:
                if wstIdx == -1:
                    wstIdx = j
                elif Memory_holes[wstIdx] < Memory_holes[j]:
                    wstIdx = j
        if wstIdx != -1:
            allocation[i] = wstIdx
            Memory_holes[wstIdx] -= Memory_req[i]
...
if __name__ == '__main__':
    ...
    print(Memory_req)
    print("Worst Fit Algorithm simulation for your data: ")
    print("\n")
    worstFit(Memory_holes, m, Memory_req, n)
